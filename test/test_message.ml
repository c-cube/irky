...
let test_extract_prefix =
  ...
  >::: [
         ...
         >:: test ~msg:"Parsing a message with a prefix"
               ...
               ~expected_output:
                 (Some "nick!user@host", "PRIVMSG destnick :abc def");
       ]
...
let test_extract_trail =
  ...
  >::: [
         "test_no_trail"
         >:: test ~msg:"Parsing a message with no trail" ~input:"PING"
...
let test_full_parser =
  ...
  >::: [
         "test_parse_ping"
         >:: test ~msg:"Parsing a PING message" ~input:"PING :abc.def"
               ~expected_output:
                 (Result.Ok
                    { M.prefix = None; command = M.PING ("abc.def", "") });
         "test_parse_privmsg"
...
let suite =
  "test_message"
  >::: [ test_extract_prefix; test_extract_trail; test_full_parser ]
